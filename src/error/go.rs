...
impl GoResult {
    ...
    /// The returned error will trigger a trap in the VM and abort contract execution immediately.
    ///
    ...
    /// This is like `into_ffi_result` but it returns the error to the contract, rather than
    /// aborting. Use only where the contract expects to handle `StdResult` from the external call.
    ///
    /// Safety: this reads data from an externally provided buffer and assumes valid utf-8 encoding
    /// Only call if you trust the code that provides output to be correct
    pub unsafe fn into_ffi_user_result<F>(self, output: Buffer, default: F) -> Result<(), FfiError>
    where
        F: Fn() -> String,
    {
        self.try_into().unwrap_or_else(|_| {
