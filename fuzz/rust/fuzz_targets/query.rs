...
use cosmwasm_vm::{
    call_instantiate_raw, call_query_raw, capabilities_from_csv,
...
fuzz_target!(|input: QueryFuzzInput| {
    ...
    // Now try to query with the fuzzed message
    let _query_result = call_query_raw(&mut instance, &raw_env, &input.query_msg);
});
