...
func TestInstantiate(t *testing.T) {
	...
	require.Equal(t, uint64(0x1021e4f), cost.UsedInternally)
...
func TestExecute(t *testing.T) {
	...
	diff = time.Since(start)
	require.NoError(t, err)
	require.Equal(t, uint64(0x1902974), cost.UsedInternally)
	t.Logf("Time (%d gas): %s\n", cost.UsedInternally, diff)
...
func TestExecuteCpuLoop(t *testing.T) {
	...
	require.NoError(t, err)
	requireOkResponse(t, res, 0)
	require.Equal(t, uint64(0xaed370), cost.UsedInternally)
	t.Logf("Time (%d gas): %s\n", cost.UsedInternally, diff)
...
func TestMultipleInstances(t *testing.T) {
	...
	assert.Equal(t, uint64(0x100dd9b), cost.UsedInternally)
	...
	require.NoError(t, err)
	requireOkResponse(t, res, 0)
	assert.Equal(t, uint64(0x1019385), cost.UsedInternally)

	// fail to execute store1 with mary
	resp := exec(t, cache, checksum, "mary", store1, api, querier, 0xeed42a)
	require.Equal(t, "Unauthorized", resp.Err)

	// succeed to execute store1 with fred
	resp = exec(t, cache, checksum, "fred", store1, api, querier, 0x18fa008)
	...
	resp = exec(t, cache, checksum, "mary", store2, api, querier, 0x18fe4be)
	require.Empty(t, resp.Err)
	require.Len(t, resp.Ok.Messages, 1)
	attributes = resp.Ok.Attributes
	require.Len(t, attributes, 2)
