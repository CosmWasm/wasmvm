...
func TestExecute(t *testing.T) {
	...
	diff = time.Since(start)
	require.NoError(t, err)
	assert.Equal(t, uint64(0x4f28bc657), cost.UsedInternally)
	t.Logf("Time (%d gas): %s\n", cost.UsedInternally, diff)
...
func TestExecuteCpuLoop(t *testing.T) {
	...
	require.NoError(t, err)
	requireOkResponse(t, res, 0)
	assert.Equal(t, uint64(0x1dc68c972), cost.UsedInternally)
	t.Logf("Time (%d gas): %s\n", cost.UsedInternally, diff)
...
func TestMultipleInstances(t *testing.T) {
	...
	resp = exec(t, cache, checksum, "fred", store1, api, querier, 0x4f087e007)
	...
	resp = exec(t, cache, checksum, "mary", store2, api, querier, 0x4f189d32f)
	require.Equal(t, "", resp.Err)
	require.Equal(t, 1, len(resp.Ok.Messages))
	attributes = resp.Ok.Attributes
	require.Equal(t, 2, len(attributes))
