...
func TestHumanAddressFailure(t *testing.T) {
	...
	igasMeter := GasMeter(gasMeter)
	_, _, err = Instantiate(cache, id, env, info, msg, &igasMeter, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	require.NoError(t, err)
	...
	res, _, err := Query(cache, id, env, query, &igasMeter3, store, badApi, &querier, 100000000, TESTING_PRINT_DEBUG)
	require.NoError(t, err)
