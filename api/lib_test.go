...
func TestInstantiate(t *testing.T) {
	...
	res, cost, err := Instantiate(cache, id, env, info, msg, &igasMeter, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	require.NoError(t, err)
...
func TestHandle(t *testing.T) {
	...
	start := time.Now()
	res, cost, err := Instantiate(cache, id, env, info, msg, &igasMeter1, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	...
	start = time.Now()
	res, cost, err = Handle(cache, id, env, info, []byte(`{"release":{}}`), &igasMeter2, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	diff = time.Now().Sub(start)
	require.NoError(t, err)
...
func TestMigrate(t *testing.T) {
	...
	res, _, err := Instantiate(cache, id, env, info, msg, &igasMeter, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	require.NoError(t, err)
	requireOkResponse(t, res, 0)

	// verifier is fred
	query := []byte(`{"verifier":{}}`)
	data, _, err := Query(cache, id, env, query, &igasMeter, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	require.NoError(t, err)
	...
	info = MockInfoBin(t, "fred")
	res, _, err = Migrate(cache, id, env, info, []byte(`{"verifier":"alice"}`), &igasMeter, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	require.NoError(t, err)
...
func TestMultipleInstances(t *testing.T) {
	...
	msg := []byte(`{"verifier": "fred", "beneficiary": "bob"}`)
	res, cost, err := Instantiate(cache, id, env, info, msg, &igasMeter1, store1, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	require.NoError(t, err)
	requireOkResponse(t, res, 0)
	...
	res, cost, err = Instantiate(cache, id, env, info, msg, &igasMeter2, store2, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	require.NoError(t, err)
	requireOkResponse(t, res, 0)
...
func exec(t *testing.T, cache Cache, id []byte, signer types.HumanAddress, store KVStore, api *GoAPI, querier Querier, gasExpected uint64) types.HandleResult {
	...
	res, cost, err := Handle(cache, id, env, info, []byte(`{"release":{}}`), &igasMeter, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	require.NoError(t, err)
...
func TestQuery(t *testing.T) {
	...
	info := MockInfoBin(t, "creator")
	msg := []byte(`{"verifier": "fred", "beneficiary": "bob"}`)
	_, _, err := Instantiate(cache, id, env, info, msg, &igasMeter1, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	require.NoError(t, err)
	...
	data, _, err := Query(cache, id, env, query, &igasMeter2, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	require.NoError(t, err)
	...
	query = []byte(`{"verifier":{}}`)
	data, _, err = Query(cache, id, env, query, &igasMeter3, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	require.NoError(t, err)
	var qres types.QueryResponse
	err = json.Unmarshal(data, &qres)
	require.NoError(t, err)
