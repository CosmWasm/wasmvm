...
func setupQueueContractWithData(t *testing.T, cache Cache, values ...int) queueData {
	...
	res, _, err := Instantiate(cache, id, env, info, msg, &igasMeter1, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
	...
	for _, value := range values {
		...
		res, _, err = Handle(cache, id, env, info, push, &gasMeter2, store, api, &querier, 100000000, TESTING_PRINT_DEBUG)
		require.NoError(t, err)
		requireOkResponse(t, res, 0)
	}
